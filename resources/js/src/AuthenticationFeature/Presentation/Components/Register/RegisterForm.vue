...
<template>
    <v-form @submit.prevent="register">
        ...
        <v-container>
            ...
            <v-text-field v-bind="testAttr('fullname-field')" v-model="store.firstAndLastName"
                :rules="store.firstAndLastNameRules" density="compact" placeholder="First and Last name"
                prepend-inner-icon="mdi-tag-outline" variant="outlined" class="pb-5" required></v-text-field>
            ...
            <v-text-field v-bind="testAttr('email-field')" v-model="store.email" :rules="store.emailRules"
                density="compact" placeholder="Email" prepend-inner-icon="mdi-email-outline" variant="outlined"
                class="pb-5" required></v-text-field>

            <div
                class="text-subtitle-1 font-weight-bold text-medium-emphasis d-flex align-center justify-space-between">
                Password
            </div>
            <v-text-field v-bind="testAttr('password-field')" v-model="store.password" :append-inner-icon="store.passwordVisible ? 'mdi-eye-off' : 'mdi-eye'
            ...
            <v-text-field v-bind="testAttr('confirm-password-field')" v-model="store.confirmPassword"
                :append-inner-icon="store.confirmPasswordVisible ? 'mdi-eye-off' : 'mdi-eye'
        ...
        <v-container>
            ...
            <div class="pt-5 d-flex flex-row justify-end">
                ...
                <v-btn v-bind="testAttr('register-btn')" class="ml-2" type="submit" :loading="store.isLoading"
